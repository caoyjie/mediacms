...
| 4 | ✅ Processing orchestration（完成） | 来源检查点执行器、MediaConvert协调、原子激活、取消和cleanup、单动作Tick与PostgreSQL reconciler | 依赖AWS dev Stack |
...
- AWS CLI的`default` profile只用于管理员部署、只读验收和读取一次运行时 Secret，不复制到镜像或容器。AWS 外部生产机使用 CloudFormation 创建的独立最小权限 IAM Runtime User；AccessKey 保存到 Secrets Manager，再写入生产机受保护的 env 文件供容器使用。
